...
describe("Courier tasks my contracts", () => {
  ...
  it("GET /api/courier/tasks/my omits task when hidden_from_courier even if pickup is confirmed", async () => {
    requireCourierAuthMock.mockResolvedValue({
      ok: true,
      user: { id: "courier-1" },
      profile: { warehouse_id: "w1", role: "courier" },
    ...
    const adminMock = createAdminFromMock({
      ...
      picking_task_units: [{ data: [], error: null }],
      picking_tasks: [{ data: [], error: null }, { data: [], error: null }],
      outbound_shipments: [
        {
          data: [
            {
