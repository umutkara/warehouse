...
export async function GET() {
  ...
  const { data: transfers, error: transfersError } = await supabaseAdmin
    ...
    .or(
      `from_warehouse_id.eq.${profile.warehouse_id},to_warehouse_id.eq.${profile.warehouse_id}`
    )
  ...
  const incoming = enriched.filter((t) => t.to_warehouse_id === profile.warehouse_id);
  const outgoing = enriched.filter((t) => t.from_warehouse_id === profile.warehouse_id);

  const warehouseIds = Array.from(
    new Set((transfers ?? []).flatMap((t) => [t.from_warehouse_id, t.to_warehouse_id]))
  );
  let warehousesById: Record<string, { id: string; name?: string | null }> = {};
  if (warehouseIds.length > 0) {
    ...
    if (!warehousesError && warehouses) {
      warehousesById = Object.fromEntries(warehouses.map((w) => [w.id, w]));
    }
  }

  return NextResponse.json({ incoming, outgoing, warehousesById });
}
