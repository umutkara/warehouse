...
function LogisticsSection() {
  return (
    <div>
      ...
      <div style={{ background: "#fff", borderRadius: 12, padding: 24, border: "2px solid #2563eb", marginBottom: 20 }}>
        <h3 style={{ fontSize: 20, fontWeight: 700, marginBottom: 10, color: "#1e40af" }}>
          📱 Готовая инструкция: Mobile Courier App
        </h3>
        <p style={{ fontSize: 13, color: "#475569", lineHeight: 1.7, marginBottom: 16 }}>
          Инструкция для курьера без технических терминов. Делайте шаги по порядку: открыть смену → взять задачу → выполнить доставку.
        </p>

        <div style={{ marginBottom: 18, padding: 14, background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 8 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#1e3a8a", marginBottom: 8 }}>
            Быстрый маршрут работы:
          </div>
          <div style={{ fontSize: 13, color: "#1e3a8a", lineHeight: 1.8 }}>
            1) Нажмите <strong>Continue</strong> / <strong>Продолжить</strong> для входа в рабочий экран.<br />
            2) Разрешите геолокацию кнопкой <strong>Allow</strong> / <strong>Разрешить</strong>.<br />
            3) Нажмите <strong>Начать смену</strong>.<br />
            4) На экране задач нажмите <strong>Перейти к задаче</strong> / <strong>Начать</strong>.<br />
            5) После передачи заказа нажмите <strong>Завершить задачу</strong> и проверьте, что статус обновился.
          </div>
        </div>

        <div style={{ marginBottom: 18, padding: 14, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 8 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#0f172a", marginBottom: 8 }}>
            Что означает каждая кнопка:
          </div>
          <div style={{ fontSize: 13, color: "#334155", lineHeight: 1.85 }}>
            • <strong>Continue / Продолжить</strong> — перейти с приветственного экрана к авторизации.<br />
            • <strong>Разрешить геолокацию (Allow)</strong> — обязательный доступ к местоположению для маршрутов и отметок.<br />
            • <strong>Начать смену</strong> — переводит вас в рабочий режим, после этого можно принимать задачи.<br />
            • <strong>Перейти к задаче</strong> — открывает текущую доставку с адресом и деталями заказа.<br />
            • <strong>Позвонить клиенту</strong> — быстрый звонок перед вручением.<br />
            • <strong>Построить маршрут</strong> — открывает навигацию до точки доставки.<br />
            • <strong>Завершить задачу</strong> — подтверждает успешную передачу и закрывает текущую доставку.<br />
            • <strong>Отмена/Проблема</strong> — использовать только если не удалось вручить (обязательно указывайте причину).
          </div>
        </div>
        ...
        <div style={{ display: "grid", gap: 16 }}>
          <div style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 12, background: "#f8fafc" }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#111827", marginBottom: 8 }}>
              Скрин 1. Вход в приложение (кнопки выделены)
            </div>
            <img
              src="/docs/courier/courier-login-annotated.png"
              alt="Экран входа mobile courier с выделенными кнопками Continue и разрешениями"
              style={{ width: "100%", borderRadius: 8, border: "1px solid #cbd5e1" }}
            />
          </div>

          <div style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 12, background: "#f8fafc" }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#111827", marginBottom: 8 }}>
              Скрин 2. Старт смены (кнопка выделена)
            </div>
            <img
              src="/docs/courier/courier-shift-annotated.png"
              alt="Экран начала смены mobile courier с выделенной кнопкой Начать смену"
              style={{ width: "100%", borderRadius: 8, border: "1px solid #cbd5e1" }}
            />
          </div>
          ...
          <div style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 12, background: "#f8fafc" }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#111827", marginBottom: 8 }}>
              Скрин 3. Экран задач (основные кнопки выделены)
            </div>
            <img
              src="/docs/courier/courier-tasks-annotated.png"
              alt="Экран задач mobile courier с выделенными рабочими кнопками"
              style={{ width: "100%", borderRadius: 8, border: "1px solid #cbd5e1" }}
            />
          </div>
        </div>
