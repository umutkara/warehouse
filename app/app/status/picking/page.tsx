...
export default function PickingStatusPage() {
  ...
  async function loadUnits() {
    ...
    try {
      const res = await fetch("/api/units/list?status=picking", { cache: "no-store" });
    ...
    <div style={{ maxWidth: 1400, margin: "0 auto", padding: 24 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 14, color: "#6b7280", lineHeight: 1.6 }}>
          OPS создал задание → заказ попал в picking (ворота). Логисты обрабатывают через ТСД и готовят к отправке в OUT. 
          Это финальная точка перед выездом со склада.
        </p>
      </div>
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 24 }}>
        <div style={{ padding: 16, background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
          <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 4 }}>Всего в Picking</div>
      ...
      {loading ? (
        ...
        <div style={{ padding: 40, textAlign: "center", color: "#9ca3af", background: "#fff", borderRadius: 12, border: "1px dashed #e5e7eb" }}>
          {searchQuery ? "Заказы не найдены" : "Нет заказов в Picking"}
        </div>
      ) : (
